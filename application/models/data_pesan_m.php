<?PHP
	class Data_pesan_m extends CI_Model
	{
		//Property
        ...
        public function set_penerima($value)
        {
            $this->penerima = $value;
        }
		...
		{
            $sql= "SELECT *
            FROM tbl_pesan  inner join tbl_pengguna  on tbl_pesan.username=tbl_pengguna.username where penerima='admin'";
            return $this->db->query($sql);
		}
        public function view_pengguna()
        {
           $sql= "SELECT *
            FROM tbl_pesan  inner join tbl_pengguna  on tbl_pesan.username=tbl_pengguna.username where penerima='admin' and tbl_pesan.username='".$this->get_username()."' ";
            return $this->db->query($sql);
        }
		...
		public function insert(){
			$sql="insert into tbl_pesan
					(username,judul,isi,penerima)
					values
					('".$this->get_username()."','".$this->get_judul()."','".$this->get_isi()."','".$this->get_penerima()."')";
           // echo $sql;
			$this->db->query($sql);
		}
			}